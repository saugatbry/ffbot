...
evo_cycle_task = None
level_up_running = False
level_up_task = None
level_up_wait_time = 30
...
def clear_cache_entry(player_id):
    ...
    async def get_account_token(self, uid, password):
        ...
        try:
            url = "https://100067.connect.in.garena.com/oauth/guest/token/grant"
            headers = {
                "Host": "100067.connect.in.garena.com",
                "User-Agent": await Ua(),
                "Content-Type": "application/x-www-form-urlencoded",
...
[FF0000]═════════════
""".strip()]
# Add these lines to one of your help menu sections
"""[00FF00]/auto_glory - Start auto guild glory play\n
[FF0000]/stop_glory - Stop auto guild glory play\n
"""
...
async def GeNeRaTeAccEss(uid, password):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=headers, data=data) as response:
            ...
            print(f"🔐 Auth response [{response.status}]: {resp_text[:300]}")
            if response.status != 200:
            ...
            if not open_id or not access_token:
                print(f"❌ Auth OK but missing fields. Got: {resp_json}")
                return (None, None)
            return (open_id, access_token)
...
async def level_up_loop(team_code, target_uid, key, iv, region, chat_type, chat_id):
    """Main level-up automation loop - joins team, starts match, waits, leaves, repeats"""
    global level_up_running, level_up_wait_time
...
"""
            ...
            if not join_success:
                error_msg = f"[B][C][FF0000]❌ Failed to join team, retrying in 3s...\n"
                await safe_send_message(chat_type, error_msg, target_uid, chat_id, key, iv)
                await asyncio.sleep(3)
                continue
            
            # Step 3: Send "ready" message
            ready_msg = f"[B][C][00FF00]✅ Bot joined team {team_code}! Starting match...\n"
            await safe_send_message(chat_type, ready_msg, target_uid, chat_id, key, iv)
            
            # Step 4: Start the match using start_auto_packet (the working start function)
            start_packet = await start_auto_packet(key, iv, region)
            
            # Spam start packet for 10 seconds to ensure it registers
            spam_duration = 10
            start_time = time.time()
            spam_count = 0
            ...
            while time.time() - start_time < spam_duration and level_up_running:
                try:
                    ...
                    print(f"Start packet send error: {e}")
                await asyncio.sleep(0.3)  # 300ms delay between packets
            
            print(f"📤 Sent {spam_count} start packets")
            
            match_started_msg = f"[B][C][FFFF00]🎮 Match start sent ({spam_count} packets)!\n"
            await safe_send_message(chat_type, match_started_msg, target_uid, chat_id, key, iv)
            
            # Step 5: Wait for match to complete
            waiting_msg = f"""[B][C][FFFF00]⏱️ MATCH IN PROGRESS...

⏳ Waiting {level_up_wait_time}s for match to complete...
🤖 Bot remains in team
...
"""
            ...
            while wait_count < level_up_wait_time and level_up_running:
                ...
                if wait_count % 10 == 0:
                    progress_msg = f"[B][C][00FF00]⏱️ {wait_count}/{level_up_wait_time}s waited...\n"
            ...
            # Step 7: Leave team
            leave_success = await level_up_leave_team(key, iv, region)
            
            if leave_success:
                leave_msg = f"[B][C][FF0000]🚪 Bot left team! Restarting cycle in 3s...\n"
            ...
            error_msg = f"[B][C][FF0000]❌ Cycle error: {str(e)[:50]}. Recovering...\n"
            await safe_send_message(chat_type, error_msg, target_uid, chat_id, key, iv)
            # Try to recover by leaving
            try:
                await level_up_leave_team(key, iv, region)
            except:
                pass
            await asyncio.sleep(3)
    
    level_up_running = False
    stop_msg = f"[B][C][FF0000]🛑 Level-up bot stopped after {cycle_count} cycles!\n"
    await safe_send_message(chat_type, stop_msg, target_uid, chat_id, key, iv)
...
async def TcPOnLine(ip, port, key, iv, AutHToKen, reconnect_delay=0.5):
    ...
    global auto_play_task, auto_play_running
        ...
        try:
            ...
            while True:
              ...
              # =================== EMOTE DETECTION ONLY ===================
                ...
                if data_hex.startswith('0500') and insquad is not None and joining_team == False and not auto_play_running:
                    try:
                ...
                # Skip invite handling entirely when auto_glory is running
                if data_hex.startswith("0500") and insquad is None and joining_team == False and not auto_play_running:
                    try:
                        packet = await DeCode_PackEt(data_hex[10:])
                        packet_json = json.loads(packet)
...
[FF0000]█▓▒░░ ENJOY THE BOT! ░░▒▓█"""
                        ...
                        joining_team = False
                        # Only reset insquad if auto_glory is NOT running
                        # (auto_guild_glory_play manages insquad reset itself)
...
# NEW ATTACK COMMAND (AUTO STOP AFTER 1 SECOND)
                       ...
                       #glory
                        # Auto play start command
                        if inPuTMsG.strip().startswith('/auto_glory'):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                err = "[B][C][FF0000]❌ Usage: /auto_glory [TeamCode]\nExample: /auto_glory 123456\n"
                                ...
                                # Start new auto play with team code
                                auto_play_running = True
                                auto_play_task = asyncio.create_task(
                                    auto_guild_glory_play(team_code, key, iv, region, LoGinDaTaUncRypTinG)
                                )

                                success_msg  = f"[B][C][00FF00]✅ Auto Glory started! Team: {team_code}\n"
                                success_msg += f"🤖 Bot joining team now...\n"
                                success_msg += f"⏳ You have 10 seconds to leave after bot joins\n"
                                success_msg += f"🎮 Match will start automatically!\n"
                                success_msg += f"⚠️ Use /stop_glory to stop\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                print(f"Started auto glory for team {team_code}")

# Auto play stop command
                        if inPuTMsG.strip() == '/stop_glory':
                            if auto_play_task and not auto_play_task.done():
                                auto_play_running = False
                                auto_play_task.cancel()
                                success_msg = f"[B][C][00FF00]✅ SUCCESS! Auto guild glory play stopped!\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                print("Auto guild glory play stopped by command")
                            else:
                                error_msg = f"[B][C][FF0000]❌ ERROR! No active auto play to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == '/stop_glory':
                            if auto_play_task and not auto_play_task.done():
                                ...
                                error_msg = f"[B][C][FF0000]❌ ERROR! No active auto play to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        # ============ LEVEL-UP BOT /lw COMMAND ============
                        if inPuTMsG.strip().startswith('/lw'):
                            # Handle /lwstop - stop level up bot
                            if inPuTMsG.strip().startswith('/lwstop'):
                                global level_up_running, level_up_task
                                level_up_running = False
                                if level_up_task and not level_up_task.done():
                                    level_up_task.cancel()
                                    level_up_task = None
                                stop_msg = f"[B][C][FF0000]🛑 Level-Up Bot STOPPED!\n"
                                await safe_send_message(response.Data.chat_type, stop_msg, uid, chat_id, key, iv)
                            
                            # Handle /lwtime - change wait time
                            elif inPuTMsG.strip().startswith('/lwtime'):
                                parts = inPuTMsG.strip().split()
                                if len(parts) < 2:
                                    error_msg = f"[B][C][FF0000]❌ Usage: /lwtime (seconds)\nExample: /lwtime 60\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    try:
                                        global level_up_wait_time
                                        new_time = int(parts[1])
                                        if new_time < 10:
                                            new_time = 10
                                        elif new_time > 300:
                                            new_time = 300
                                        level_up_wait_time = new_time
                                        success_msg = f"[B][C][00FF00]✅ Level-up wait time set to {level_up_wait_time} seconds!\n"
                                        await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                    except ValueError:
                                        error_msg = f"[B][C][FF0000]❌ Invalid number! Use: /lwtime 30\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            
                            # Handle /lw (team_code) - start level up bot
                            else:
                                parts = inPuTMsG.strip().split()
                                if len(parts) < 2:
                                    error_msg = f"""[B][C][FF0000]❌ Usage: /lw (TeamCode)
[FFFFFF]Example: /lw ABC123

[00FFFF]📋 Level-Up Bot Commands:
[00FF00]/lw (code) [FFFFFF]- Start level-up bot
[00FF00]/lwstop [FFFFFF]- Stop level-up bot
[00FF00]/lwtime (sec) [FFFFFF]- Set wait time (default: 30s)
"""
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    team_code = parts[1]
                                    
                                    # Stop any existing level-up task
                                    level_up_running = False
                                    if level_up_task and not level_up_task.done():
                                        level_up_task.cancel()
                                        await asyncio.sleep(1)
                                    
                                    # Start new level-up automation
                                    level_up_running = True
                                    
                                    start_msg = f"""[B][C][00FF00]🚀 LEVEL-UP BOT STARTED!

[FFFFFF]📋 Settings:
[00FFFF]• Team Code: {team_code}
[00FFFF]• Wait Time: {level_up_wait_time}s between cycles
[00FFFF]• Auto: Join → Start Match → Wait → Leave → Repeat

[FFFF00]💡 Use /lwstop to stop the bot
[FFFF00]💡 Use /lwtime (sec) to change wait time
"""
                                    await safe_send_message(response.Data.chat_type, start_msg, uid, chat_id, key, iv)
                                    
                                    # Launch the level-up loop as background task
                                    level_up_task = asyncio.create_task(
                                        level_up_loop(team_code, uid, key, iv, region, response.Data.chat_type, chat_id)
                                    )
                                    print(f"🚀 Level-up task started for team: {team_code}")
...
[00FFFA]╚═•══•════════════════•══•═╝"""
        ...
        await asyncio.sleep(reconnect_delay)
async def create_solo_br_start_packet(key, iv, region):
    """Solo BR start packet - type 9 with solo mode flag"""
    fields = {
        1: 9,
        2: {
            1: 12480598706,
...
async def switch_to_solo_br(bot_uid, key, iv, region):
    """Switch bot to Solo BR mode before starting match"""
    if region.lower() == "ind":
        pt = "0514"
    elif region.lower() == "bd":
        pt = "0519"
        ...
        pt = "0515"
    fields = {1: 17, 2: {1: int(bot_uid), 2: 1, 3: 1, 4: 43, 5: "\x0b", 8: 1, 19: 1}}
    return await GeneRaTePk((await CrEaTe_ProTo(fields)).hex(), pt, key, iv)

async def solo_br_start_packet(key, iv, region):
    """Solo BR specific start - different from squad start_auto_packet"""
    if region.lower() == "ind":
        pt = "0514"
    elif region.lower() == "bd":
        pt = "0519"
        ...
        pt = "0515"
    # Type 9 = start, field 2.1 = match context for solo BR
    fields = {1: 9, 2: {1: 1}}
    return await GeneRaTePk((await CrEaTe_ProTo(fields)).hex(), pt, key, iv)


async def auto_guild_glory_play(team_code, key, iv, region, login_data):
    """Auto Glory: own room -> solo BR mode -> spam start -> wait -> repeat. team_code unused."""
    global auto_play_running, insquad, current_match_id, match_start_time

    bot_uid = getattr(login_data, 'AccountUID', None) or getattr(login_data, 'account_uid', 14010319252)
    print(f"\U0001f3c6 [Auto Glory] Solo BR STARTED | Bot UID: {bot_uid}")
    cycle = 0

    while auto_play_running:
        try:
            cycle += 1
            print(f"\n\U0001f504 [Auto Glory] Cycle #{cycle}")

            # Step 1: Leave any existing squad cleanly
            print("\U0001f6aa [Auto Glory] Leaving squad...")
            try:
                leave_pkt = await leave_squad(key, iv, region)
                await SEndPacKeT(whisper_writer, online_writer, 'OnLine', leave_pkt)
            except Exception as e:
                print(f"\u26a0\ufe0f Leave error (ok): {e}")
            await asyncio.sleep(1)

            if not auto_play_running:
                break

            # Step 2: Open own fresh solo room
            print("\U0001f3e0 [Auto Glory] Opening own room...")
            try:
                open_pkt = await OpEnSq(key, iv, region)
                await SEndPacKeT(whisper_writer, online_writer, 'OnLine', open_pkt)
            except Exception as e:
                print(f"\u26a0\ufe0f Open room error: {e}")
            await asyncio.sleep(2)

            if not auto_play_running:
                break

            # Step 3: Switch to Solo BR mode
            print("\U0001f3ae [Auto Glory] Switching to Solo BR mode...")
            try:
                solo_switch = await switch_to_solo_br(bot_uid, key, iv, region)
                await SEndPacKeT(whisper_writer, online_writer, 'OnLine', solo_switch)
                print("\u2705 [Auto Glory] Solo BR mode set!")
            except Exception as e:
                print(f"\u26a0\ufe0f Solo switch error: {e}")
            await asyncio.sleep(1)

            if not auto_play_running:
                break

            # Step 4: Spam BOTH solo_br_start_packet AND start_auto_packet alternately
            print(f"\U0001f3ae [Auto Glory] Spamming Solo BR start for {START_SPAM_DURATION}s...")
            pkt_a = await solo_br_start_packet(key, iv, region)
            pkt_b = await start_auto_packet(key, iv, region)
            match_start_time = time.time()
            spam_start = time.time()
            spam_count = 0

            while time.time() - spam_start < START_SPAM_DURATION and auto_play_running:
                try:
                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', pkt_a)
                    spam_count += 1
                    await asyncio.sleep(START_SPAM_DELAY)
                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', pkt_b)
                    spam_count += 1
                    if spam_count % 20 == 0:
                        print(f"\U0001f4e4 [Auto Glory] {spam_count} packets sent...")
                except Exception as e:
                    print(f"\u26a0\ufe0f Send error: {e}")
                await asyncio.sleep(START_SPAM_DELAY)

            print(f"\u2705 [Auto Glory] {spam_count} packets sent. Waiting {WAIT_AFTER_MATCH_SECONDS}s...")

            # Step 5: Wait - no leave
            waited = 0
            while waited < WAIT_AFTER_MATCH_SECONDS and auto_play_running:
                await asyncio.sleep(1)
                waited += 1

            current_match_id = None
            print(f"\U0001f501 [Auto Glory] Cycle #{cycle} complete. Next cycle...")

        except asyncio.CancelledError:
            print("\U0001f6d1 [Auto Glory] Cancelled.")
            break
        except Exception as e:
            print(f"\u274c [Auto Glory] Cycle error: {e}")
            traceback.print_exc()
            await asyncio.sleep(5)

    auto_play_running = False
    print("\U0001f3c1 [Auto Glory] Auto play ended.")
...
async def MaiiiinE():
    ...
    # Split IPs and ports
    # Use rsplit to safely handle IPv6 addresses or extra colons — port is always last
