...
def clear_cache_entry(player_id):
    ...
    async def get_account_token(self, uid, password):
        ...
        try:
            url = "https://100067.in.connect.garena.com/oauth/guest/token/grant"
            headers = {
                "Host": "100067.in.connect.garena.com",
                "User-Agent": await Ua(),
                "Content-Type": "application/x-www-form-urlencoded",
                       ...
                       #glory
                        ...
                        if inPuTMsG.strip().startswith('/auto_glory'):
                            ...
                            auto_play_task = asyncio.create_task(
                                auto_guild_glory_play(key, iv, region, LoGinDaTaUncRypTinG)
                            )
    
                            # SUCCESS MESSAGE
                            success_msg = f"[B][C][00FF00]✅ SUCCESS! Auto guild glory play started!\n"
                            success_msg += f"🎮 Bot will automatically join matches and play\n"
                            success_msg += f"⏱️ Running continuously until stopped\n"
                            success_msg += f"⚠️ Use /stop_glory to stop the auto play\n"
                            await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            print(f"Started auto guild glory play")
                        except Exception as e:
